 ...
 * @Last Modified by: 雪糕
 * @Last Modified time: 2020-02-16 17:04:06
 */
const LOG_TAG = {
	lanbo: "lanbo",
	NetWork: "NetWork",
...
namespace lanbo {
	export namespace logger {
		...
		export function init() {
			_model = new Model();
			lanbo.logger.log(LOG_TAG.lanbo, `初始化logger模块`);
		}
		...
		class Model extends BaseModel {
			...
			protected async onInit() {
				...
				//native暂时不支持 会启动报错
				if (!lanbo.Capabilities.isRuntime && this.isSupportIndexedDB) {
				...
				//启用了worker
				if (lanbo.net.worker) {
					lanbo.net.postMessageToWorker("LoggerInit", tableName);
				} else {
					if (lanbo.net.webSocketWorker) {
						lanbo.net.webSocketWorker.writeLog(tableName);
					}
				}
			...
			private get uploadLogTime() {
				return lanbo.time.getStartupTime().toString();
			}
			...
			public saveData(data: any) {
				...
				//启用了worker
				if (lanbo.net.worker) {
					lanbo.net.postMessageToWorker("Logger", data);
				} else {
					if (lanbo.net.webSocketWorker) {
						lanbo.net.webSocketWorker.writeLog(data);
					}
				}
			}
		...
		function addLogHead(tag: string, info: string, needTime: boolean = true): string {
			if (needTime) {
				return `[${tag}]${new Date().format()}\t${info}`;
			} else {
				return `[${tag}]\t${info}`;
			}
		}
